...
async function processIntake(job) {
  ...
  const r = await one(
    `SELECT r.*,u.course AS uploader_course,u.semester AS uploader_semester
     FROM resources r
  ...
  const isTemporary =
    path.resolve(temp) !== path.resolve(r.stored_path);
  ...
  try {
    ...
    if (!text.trim()) {
      throw new Error(
        'No readable text could be extracted. Upload a text-based document or a clearer scan.'
      );
    }

    const subjects = (
      await query(
        'SELECT name FROM subjects WHERE course=?',
        [r.course]
      )
    ).map(x => x.name);

    const raw = await callAI(
      `Classify an academic upload. Return ONLY JSON: {"subject":"provided subject or UNSURE","subjectConfidence":0-1,"contentType":"Notes|PYQ|Assignment|Cheat Sheet|Lab Manual|Syllabus","examType":"Mid-Sem|End-Sem|Quiz|University|","paperYear":"YYYY or ","professor":"","suggestedTitle":"","reasoning":""}. Never invent a subject.`,
      `Course: ${r.course}
...
${text.slice(0, 6000)}`
    ...
    const subject = subjects.includes(a.subject)
      ? a.subject
      : '';

    const ct = contentTypes.includes(a.contentType)
      ? a.contentType
      : 'Notes';

    const ex =
      ct === 'PYQ' && examTypes.includes(a.examType)
        ? a.examType
        : '';

    const year =
      /^(19|20)\d{2}$/.test(String(a.paperYear || ''))
        ? String(a.paperYear)
        : '';

    const title = String(
      a.suggestedTitle || r.file_name
    )
      .trim()
      .slice(0, 180);
    ...
    if (subject) {
      duplicates = await query(
        `SELECT id,title,subject,
         ...
         downloads,
         MATCH(title,professor,excerpt)
         AGAINST(? IN NATURAL LANGUAGE MODE) relevance
         FROM resources
         WHERE id<>?
         AND course=?
         AND semester=?
    ...
    await query(
      ...
      [
        ...
        ex,
        year,
        String(a.professor || '').slice(0, 120),
        title,
        text.slice(0, 1500),
        JSON.stringify(a),
        r.id
    ...
    await query(
      `INSERT INTO agent_logs
       (agent,user_id,input_summary,actions_json,output_summary,status,duration_ms,created_at)
       VALUES(?,?,?,?,?,?,?,?)`,
      [
        'intake_agent',
        job.user_id,
        ...
        JSON.stringify({
          analysis: a,
          duplicateCandidates: duplicates.map(d => d.id)
        }).slice(0, 4000),
        'success',
        now() - started,
        now()
    ...
    // Only delete temporary S3 copies.
    // Never delete the permanent local upload.
    if (isTemporary) {
      await fs.rm(temp, {
        force: true
      }).catch(() => {});
